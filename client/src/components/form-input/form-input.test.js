...
describe("FormInput component", () => {
  ...
  it("should call handleChange method when input changes", () => {
    wrapper.find(`[data-test='FormInputContainer']`).simulate("change");

    expect(mockHandleChange).toHaveBeenCalled();
  });

  it("should render FormInputLabel if there is a label", () => {
    expect(wrapper.exists(`[data-test='FormInputLabel']`)).toBe(true);
  });
  ...
  it("should not render FormInputLabel if there is no label", () => {
    ...
    expect(newWrapper.exists(`[data-test='FormInputLabel']`)).toBe(false);
  });
});
